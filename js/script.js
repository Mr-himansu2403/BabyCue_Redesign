...
// Initialize mobile navigation
const mobileNavigationEnhanced = new MobileNavigation();

// Make it globally available
window.mobileNavigation = window.mobileNavigation || mobileNavigationEnhanced;
...
const initAchievementsAutoFlow = () => {
  const achievementsGallery = document.getElementById('achievementsGallery');
  if (!achievementsGallery || achievementsGallery.dataset.autoflowInitialized === 'true') return;

  const cards = Array.from(achievementsGallery.querySelectorAll('.achievement-card'));
  if (cards.length === 0) return;

  achievementsGallery.dataset.autoflowInitialized = 'true';

  // Disable conflicting CSS animation paths and keep cards visible immediately.
  achievementsGallery.style.animation = 'none';
  achievementsGallery.style.webkitAnimation = 'none';
  achievementsGallery.style.mozAnimation = 'none';
  achievementsGallery.style.oAnimation = 'none';
  achievementsGallery.style.justifyContent = 'flex-start';
  achievementsGallery.style.transform = 'translate3d(0, 0, 0)';

  // If markup already contains a duplicated sequence, trim to the first full set.
  const cardSignature = (card) => {
    const img = card.querySelector('img');
    const title = card.querySelector('.achievement-title');
    return `${img ? (img.getAttribute('src') || '') : ''}|${title ? title.textContent.trim() : ''}`;
  };

  const firstSignature = cardSignature(cards[0]);
  const duplicateStartIndex = cards.findIndex((card, index) => index > 0 && cardSignature(card) === firstSignature);
  if (duplicateStartIndex > 0) {
    achievementsGallery.innerHTML = cards
      .slice(0, duplicateStartIndex)
      .map((card) => card.outerHTML)
      .join('');
  }

  // Build one exact clone set for seamless looping.
  const baseMarkup = achievementsGallery.innerHTML;
  achievementsGallery.insertAdjacentHTML('beforeend', baseMarkup);

  let baseSpeedPxPerSec = 42;
  let offsetX = 0;
  let lastTimestamp = 0;
  ...
  const syncRunningState = () => {
    isRunning = !isManuallyPaused && !isHoverPaused;
  };

  const getLoopWidth = () => achievementsGallery.scrollWidth / 2;
  ...
  const tick = (timestamp) => {
    ...
    if (isRunning) {
      ...
      offsetX -= baseSpeedPxPerSec * dt;

      if (-offsetX >= loopWidth && loopWidth > 0) {
        offsetX += loopWidth;
      }

      achievementsGallery.style.transform = `translate3d(${offsetX}px, 0, 0)`;
    }

    requestAnimationFrame(tick);
  };

  const allCards = achievementsGallery.querySelectorAll('.achievement-card');
  ...
  if (playPauseBtn) {
    ...
    playPauseBtn.addEventListener('click', () => {
      ...
      syncRunningState();
      if (playIcon) playIcon.style.display = isRunning ? 'none' : 'inline';
      if (pauseIcon) pauseIcon.style.display = isRunning ? 'inline' : 'none';
    });
  }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initAchievementsAutoFlow);
} else {
  initAchievementsAutoFlow();
}

window.addEventListener('load', initAchievementsAutoFlow);
...
/* ============================================================
   LEGAL MODALS & CONTACT FORM LOGIC
   ============================================================ */
